...
Not like Square, Toast, or Clover — avoid the cluttered POS-grid aesthetic, the feature-dense sidebars, or the enterprise-gray color palettes that feel like accounting software. Not like Stripe's default Checkout — too developer-centric, too minimal, lacks brand warmth. Not like Shopify Payments — too ecommerce-native, built for inventory-first businesses, not quick-service or booking flows. Lume should feel like a fintech product that a chef or salon owner would actually choose because it looks like it belongs in their world.

## Design Principles

1. **One clear action per screen**: Every checkout state should have one dominant button. If a user has to decide between two equal-weight actions to complete a payment, it fails.
2. **Speed is the feature**: The checkout must feel faster than the alternative — a card machine, a third-party link, a paper menu. If reaching the pay button takes more than three taps from order summary, it fails.
...
WCAG AA minimum. Touch targets 44×44px minimum — the primary checkout button must be thumb-reachable in the lower half of a mobile screen, since most orders happen on phones mid-task (at a table, at a counter, on a couch). Color must not be the sole indicator of state; the hot pink brand color cannot carry meaning alone — pair it with label or icon. Sufficient contrast on dark-background sections (#17120e) is mandatory; pink-on-dark text must be tested, not assumed.
